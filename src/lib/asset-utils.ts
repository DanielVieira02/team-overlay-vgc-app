...
export function getPokemonIconPath(pokemonName: string): string {
  ...
  for (const variation of variations) {
    ...
    const titleCase =
      variation.charAt(0).toUpperCase() + variation.slice(1).toLowerCase();
    ...
    for (const caseVariation of [lowerCase, upperCase, titleCase]) {
      if (pokemonTranslator[caseVariation as keyof typeof pokemonTranslator]) {
        const spriteId =
          pokemonTranslator[caseVariation as keyof typeof pokemonTranslator];
  ...
  console.warn(
    `Pokemon icon not found for: "${pokemonName}". Tried variations:`,
    variations,
  );
...
export function getItemIconPath(itemName?: string): string {
  ...
  for (const variation of variations) {
    ...
    for (const caseVariation of [titleCase, lowerCase]) {
      if (itemTranslator[caseVariation as keyof typeof itemTranslator]) {
        const itemId =
          itemTranslator[caseVariation as keyof typeof itemTranslator];
        return `/assets/ItemsIcons/${itemId}.png`;
      }
    }
  }
  ...
  console.warn(
    `Item icon not found for: "${itemName}". Tried variations:`,
    variations,
  );
...
export function getTypeIconPath(typeName: string, isTera?: boolean): string {
  ...
  if (isTera) return `/assets/TypeIcons/TeraType/${cleanType}.png`;
